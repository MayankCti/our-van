...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(authLogin.fulfilled, (state, action) => {
      ...
      const user = data?.technician || data?.user || (typeof data === "object" && !data?.token ? data : null);
    ...
    builder.addCase(authGetProfile.fulfilled, (state, action) => {
      const user = action?.payload?.data || action?.payload?.technician || action?.payload?.user;
      if (user) {
        state.user = user;
        pipSaveProfile(user);
      }
